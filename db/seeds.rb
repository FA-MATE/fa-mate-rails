...
}.map.with_index(1) do |kv, order_no|
  name, conditions = kv
  ConditionGroup.create(name:, order_no:).tap do |condition_group|
    conditions.each.with_index(1) do |condition_name, condition_order_no|
      Condition.create(condition_group_id: condition_group.id, name: condition_name, order_no: condition_order_no)
    end
  end
end
...
categories = {
  ...
  Category.create(name:, order_no:).tap do |category|
    sub_categories.each.with_index(1) do |sub_category_name, sub_category_name_order_no|
      SubCategory.create(category_id: category.id, name: sub_category_name, order_no: sub_category_name_order_no)
    end
  end
end
...
tag_groups = {
  ...
  name, tags = kv
  TagGroup.create(name:, order_no:).tap do |tag_group|
    tags.each.with_index(1) do |tag_name, tag_order_no|
      Tag.create(tag_group_id: tag_group.id, name: tag_name, order_no: tag_order_no)
    end
  end
end
